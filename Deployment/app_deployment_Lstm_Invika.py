...
#model = tf.keras.models.load_model('bert_finetuned_model.pth')
...
def index():
    if request.method == 'POST':
        ...
        processed_text = process_input_sentence(input_text)

        if 'submit_lstm' in request.form:
            prediction = model.predict(processed_text)
            prediction_label = "REAL" if prediction >= 0.6 else "FAKE"
            return render_template('index.html', prediction=prediction_label, model_type="LSTM", input_text=input_text)

        elif 'submit_bert' in request.form:
            prediction = 0.7  
            prediction_label = "REAL" if prediction >= 0.6 else "FAKE"
            return render_template('index.html', prediction=prediction_label, model_type="Other Model", input_text=input_text)

    return render_template('index.html', prediction=None, model_type=None)
